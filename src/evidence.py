"""
Calculation of the marginal likelihood using the Multi-histogram method
(or Weighted Histogram Analysis Method) from log-likelihood samples at multiple temperatures
obtained from Replica Exchange method (a.k.a. Exchange Monte Carlo or Parallel Tempering) simulation
"""
...
class SolveWHAM:
    ...
    def E_edge_M(self) -> FloatArray:
        return np.linspace(
            self.E_NK.min() - self.d / 2, self.E_NK.max() + self.d / 2, self.M + 1
        )
    ...
    def h_KM(self):
        return np.array(
            [
                np.histogram(self.E_NK[k], bins=self.E_edge_M)[0]
    ...
    def lAY_to_Ag(self, lAY_K: FloatArray) -> FloatArray:
    ...
    def Ag_to_lAY(self, Ag_M: FloatArray) -> FloatArray:
        return np.array(
            [
    ...
    def Ag_to_lAZ(self, Ag_M: FloatArray) -> FloatArray:
        return np.array(
            [
                -beta * self.E_min
                + np.log((Ag_M * np.exp(-beta * (self.E_M - self.E_min))).sum())
    ...
    def dist(self, beta: float) -> FloatArray:
        with np.errstate(divide="ignore"):
            ...
            midrange = (
                logL_M[np.isfinite(logL_M)].min() + logL_M[np.isfinite(logL_M)].max()
            ) / 2
...
class SolveEvidence:
    ...
    def range_groups(self) -> List[Tuple[int, int]]:
        return [
            (s, t)
            for s in range(0, self.K, self.W - 1)
            if s + 1 != (t := min(s + self.W, self.K))
        ]
    ...
    def __post_init__(self):
        ...
        self.log_Z_K = np.array(
            sum([log_Z_S[:-1].tolist() for log_Z_S in self.log_Z_groups], [])
            + [self.log_Z_groups[-1][-1]]
        )
    ...
    def pred_dist(self, new_betas) -> Tuple[List[FloatArray], List[FloatArray]]:
        ...
        for s, sub_solver in enumerate(self.solvers):
            if s == 0:
                ...
                betas = new_betas[
                    (new_betas >= sub_solver.beta_K.min())
                    & (new_betas <= sub_solver.beta_K.max())
                ]
            stats: List = []
            for beta in betas:
                ...
                stats.append(
                    [
                        *[
                            poly_inverse(p, wide_E_M, wide_CP_M)  # type: ignore
        ...
        log_Z_KB = p.map(
            ...
            [
                (
                    ...
                    else hash((random_state, r)) % np.iinfo(int).max,
                )
...
if __name__ == "__main__":
    ...
    log_Z_KB, solvers = calc_evidence_bootstrap(
        beta_K, ll_n_k, W=4, n_bins_wham=10**4, n_bootstrap=10, verbose=True
    )
    # parallel processing
    ...
    plt.errorbar(beta_K, means, yerr=errs, fmt="o-", capsize=3, markersize=5)
    ...
    plt.xticks(
        sorted({x for x in plt.xticks()[0] if pow_floor(beta_K.min()) <= x <= 1} | {1})  # type: ignore
    )
    plt.yticks([y for y in plt.yticks()[0] if not (0 < abs(y) < 1)])  # type: ignore
    plt.show()
